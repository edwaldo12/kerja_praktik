(function ($) {
        ...
        initIcons: function () {
            ...
            $(".panel").each(function (i, key) {
                if (!$(key).parent().hasClass("panel-group")) {
                    ...
                    if ($(key).find(".panel-controls").length > 0)
                        $panelControls = $(key).find(".panel-controls");

                    //Collapsable Icon
                    if (dataAttrs["panelCollapsable"] != undefined) {
                        if (dataAttrs["panelCollapsable"])
                            $panelControls.append($this.collapsableIcon());
                    } else {
                        if (configs.controls.collapsable)
                            $panelControls.append($this.collapsableIcon());
                    }

                    //Fullscreen Icon
                    if (dataAttrs["panelFullscreen"] != undefined) {
                        if (dataAttrs["panelFullscreen"])
                            $panelControls.append($this.fullscreenIcon());
                    } else {
                        if (configs.controls.fullscreen)
                            $panelControls.append($this.fullscreenIcon());
                    }

                    //Close Icon
                    if (dataAttrs["panelClose"] != undefined) {
                        if (dataAttrs["panelClose"])
                            $panelControls.append($this.closeIcon());
                    } else {
                        if (configs.controls.close)
                            $panelControls.append($this.closeIcon());
                    }
    ...
    $.AdminBSB.leftSideBar = {
        ...
        setVerticalScrollBar: function () {
            ...
            if ($this.isFixed()) {
                var $menu = $(".metismenu");
                var height = $.AdminBSB.navbar.isFixed()
        ...
        setSidebarHeight: function () {
            ...
            var sidebarNewHeight =
                Math.max(sidebarHeight, contentHeight, docHeight) +
    ...
    $.AdminBSB.rightSideBar = {
        ...
        setVerticalScroll: function () {
            ...
            var height =
                $(window).height() - $(".right-sidebar .nav-tabs").height();
